    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          ...
          <p className="text-slate-600 text-lg">כל השחקנים בליגה ({players.length})</p>
        </motion.div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-white/60 border border-slate-200/60 rounded-lg pr-10 pl-3 py-2 text-sm"
            />
          </div>
          <select value={teamFilter} onChange={e => setTeamFilter(e.target.value)} className="bg-white/60 border border-slate-200/60 rounded-lg px-3 py-2 text-sm">
            <option value="all">כל הקבוצות</option>
            {teams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
          </select>
          <select value={positionFilter} onChange={e => setPositionFilter(e.target.value)} className="bg-white/60 border border-slate-200/60 rounded-lg px-3 py-2 text-sm">
            <option value="all">כל העמדות</option>
            <option value="Field Player">שחקן שדה</option>
            <option value="Goalkeeper">שוער</option>
          </select>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-white/60 border border-slate-200/60 rounded-lg px-3 py-2 text-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filteredPlayers.map((player, index) => (
            <motion.div
              ...
              className="bg-white/60 backdrop-blur-sm border border-slate-200/60 rounded-xl p-4 hover:shadow-md transition-shadow"
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-1">
                  ...
                  {player.is_core && <span className="text-xs px-1.5 py-0.5 rounded bg-orange-100 text-orange-700">ליבה</span>}
              ...
              <div className="grid grid-cols-4 gap-2 text-center">
                <div className="bg-slate-50 rounded-lg p-2">
                  ...
                  <p className="text-[10px] text-slate-500">שערים</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-2">
                  <p className="text-lg font-bold text-slate-700">{player.games_played || 0}</p>
                  <p className="text-[10px] text-slate-500">משחקים</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-2">
                  <p className="text-lg font-bold text-blue-600">{player.blue_cards || 0}</p>
                  <p className="text-[10px] text-slate-500">כחולים</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-2">
                  <p className="text-lg font-bold text-red-600">{player.red_cards || 0}</p>
                  <p className="text-[10px] text-slate-500">אדומים</p>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        {filteredPlayers.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-slate-600">אין שחקנים תואמים</h3>
          </div>
        )}
      </div>
    </div>
