    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          ...
          <p className="text-slate-600 text-lg">שלב הגמר - עונת 2024-25</p>
        </motion.div>
        ...
        <div className="bg-white/60 backdrop-blur-sm border border-slate-200/60 rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Playoff Round */}
            <div>
              <h3 className="text-sm font-semibold text-slate-500 mb-4 text-center">סיבוב ראשון (פלייאוף)</h3>
              <div className="space-y-4">
                {playoffSeries.map((series, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{series.name}</span>
                      <div className="mt-3 space-y-2">
                        <div className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                          <div className="w-6 h-6 rounded-full" style={{ backgroundColor: getTeamColor(series.team1?.id) }} />
                          <span className="font-semibold text-sm flex-1">{series.team1?.name}</span>
                          <span className="text-xs text-slate-500">#{series.pos1}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                          ...
                          <span className="font-semibold text-sm flex-1">{series.team2?.name}</span>
                          <span className="text-xs text-slate-500">#{series.pos2}</span>
                        </div>
                      </div>
                      ...
                      {seriesGames.length > 0 && (
                        <div className="mt-3 space-y-1">
                          {seriesGames.map(g => (
                            <div key={g.id} className="flex items-center justify-between text-xs p-2 bg-blue-50 rounded">
                              <span>משחק {g.series_game}</span>
                              <span>{format(new Date(g.game_date), "d/M/yy")}</span>
                              {g.status === 'completed' ? (
                                <span className="font-bold">{g.home_score} - {g.away_score}</span>
                              ) : (
                                <span className="text-blue-600">{g.status === 'scheduled' ? 'מתוכנן' : g.status}</span>
                              )}
                            </div>
                          ))}
                        </div>
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-500 mb-4 text-center">חצי גמר</h3>
              <div className="space-y-4">
                {/* Auto-qualified team */}
                {firstPlace && (
                  <div className="border-2 border-yellow-300 rounded-xl p-4 bg-gradient-to-br from-yellow-50 to-white">
                    <div className="flex items-center gap-2 mb-2">
                      <Crown className="w-4 h-4 text-yellow-600" />
                      <span className="text-xs font-semibold text-yellow-700">מקום ראשון</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full" style={{ backgroundColor: firstPlace.primary_color || '#f97316' }} />
                      <span className="font-bold">{firstPlace.name}</span>
                    </div>
                    <p className="text-xs text-yellow-600 mt-2">עלתה אוטומטית ל-Final Four</p>
                  </div>
                )}
            ...
            {/* Final */}
            <div>
              <h3 className="text-sm font-semibold text-slate-500 mb-4 text-center">גמר</h3>
              <div className="border border-dashed border-orange-300 rounded-xl p-8 text-center bg-gradient-to-br from-orange-50/50 to-white">
                <Trophy className="w-12 h-12 text-orange-400 mx-auto mb-3" />
                <p className="text-slate-400 text-sm">הגמר טרם נקבע</p>
                <p className="text-xs text-slate-300 mt-1">ייקבע לאחר סיום הפלייאוף</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-slate-200/60 rounded-xl p-6">
            ...
            <div className="grid gap-3">
              {playoffGames.sort((a, b) => new Date(a.game_date) - new Date(b.game_date)).map(game => (
                <div key={game.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-100">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full" style={{ backgroundColor: getTeamColor(game.home_team_id) }} />
                    <span className="font-semibold text-sm">{getTeamName(game.home_team_id)}</span>
                    <span className="text-slate-400 text-sm">vs</span>
                    <span className="font-semibold text-sm">{getTeamName(game.away_team_id)}</span>
                    <div className="w-8 h-8 rounded-full" style={{ backgroundColor: getTeamColor(game.away_team_id) }} />
                  </div>
                  <div className="text-left text-sm">
                    <p className="font-medium">{format(new Date(game.game_date), "d/M/yy")}</p>
